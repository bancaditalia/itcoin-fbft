...
BitcoinRpcWallet::BitcoinRpcWallet(const itcoin::FbftConfig& conf, transport::BtcClient& bitcoind)
  ...
  BOOST_LOG_TRIVIAL(debug) << str(boost::format("R%1% BitcoinRpcWallet will sign using pubkey address %2%.") %
                                  m_conf.id() % m_pubkey_address);
}

void BitcoinRpcWallet::AppendSignature(Message& message) const {
  if (message.sender_id() != m_conf.id()) {
    string error_msg = str(boost::format("R%1% BitcoinRpcWallet cannot sign message with sender_id = %2%.") %
                           m_conf.id() % message.sender_id());
  ...
  BOOST_LOG_TRIVIAL(trace) << str(boost::format("R%1% BitcoinRpcWallet signing message with digest = %2%.") %
                                  m_conf.id() % msg_digest);
...
bool BitcoinRpcWallet::VerifySignature(const Message& message) const {
  ...
  string msg_pubkey_address = m_conf.replica_set_v().at(message.sender_id()).p2pkh();

  return m_bitcoind.verifymessage(msg_pubkey_address, msg_sig, msg_digest);
}
