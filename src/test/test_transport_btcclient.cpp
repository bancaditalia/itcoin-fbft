...
const std::string helloSignature{
    "IOzKScUZOETfXTwQFFNP+xMigAXMMTEZhLglGAe197ldMC15bnokhgMPR/l1QrCsJIgK0gCUW1NyemCy1ChHhfU="};
...
BOOST_FIXTURE_TEST_CASE(test_combinepsbt, BtcClientFixture) {
  ...
  list.append(psbtBase64);
  BOOST_LOG_TRIVIAL(info) << "Sending a combinepsbt request with arguments " << psbtBase64 << " and "
                          << psbtBase64;
...
BOOST_DATA_TEST_CASE_F(BtcClientFixture, test_finalizepsbt, bdata::make({true, false}), extractHex) {
  auto psbtBase64 = buildEmptyPsbt();
  Json::Value response;

  BOOST_LOG_TRIVIAL(info) << "Sending a finalizepsbt request with arguments psbt=" << psbtBase64
                          << " and extract=" << extractHex;
...
BOOST_FIXTURE_TEST_CASE(test_submitblock_fails, BtcClientFixture) {
  ...
  auto isInvalidBlockError = [](jsonrpc::JsonRpcException e) -> bool {
    if (e.GetCode() != expectedCode) {
      BOOST_LOG_TRIVIAL(error) << "Wrong code in exception: " + std::to_string(e.GetCode()) + " instead of " +
                                      std::to_string(expectedCode);
      return false;
    }
  ...
  BOOST_CHECK_EXCEPTION(bitcoind0.submitblock("INVALID BLOCK"), jsonrpc::JsonRpcException,
                        isInvalidBlockError);
}
...
BOOST_FIXTURE_TEST_CASE(testblockvalidity, BtcClientFixture) {
  ...
  BOOST_CHECK_EXCEPTION(bitcoind0.testblockvalidity("INVALID BLOCK"), jsonrpc::JsonRpcException,
                        isInvalidBlockError);
  BOOST_CHECK_EXCEPTION(bitcoind0.testblockvalidity("INVALID BLOCK", true), jsonrpc::JsonRpcException,
                        isInvalidBlockError);
  BOOST_CHECK_EXCEPTION(bitcoind0.testblockvalidity("INVALID BLOCK", false), jsonrpc::JsonRpcException,
                        isInvalidBlockError);
}
