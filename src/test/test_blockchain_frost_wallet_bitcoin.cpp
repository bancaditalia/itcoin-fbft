...
struct Replica4Fixture : BitcoinInfraFixture {
  Replica4Fixture() : BitcoinInfraFixture() {}
};
...
BOOST_FIXTURE_TEST_CASE(test_blockchain_frost_wallet_bitcoin_00, Replica4Fixture) {
  ...
  {
    BOOST_LOG_TRIVIAL(debug) << "Testing GetSignatureShare: generate signature share with below threshold "
                                "presignatures (expect no error)";
    std::vector<uint32_t> signers;
    std::string pre_signatures = "";
    CBlock block;
  ...
  // GetSignatureShare: check that each participant generates its signature_response
  {
    BOOST_LOG_TRIVIAL(debug)
        << "Testing GetSignatureShare: each participant should generate its signature_response";
    std::vector<uint32_t> signers;
    std::string pre_signatures = "";
    CBlock block;
  ...
  { // Assumption: we generated keys to allow a 2-4 threshold signature. Two participants can generate a valid
    // signature.
  ...
  { // Assumption: we generated keys to allow a 2-4 threshold signature. Two participants can generate a valid
    ...
    BOOST_CHECK_THROW(frost_wallets[0]->FinalizeBlock(block, pre_signatures, signature_shares),
                      std::runtime_error);
  }
