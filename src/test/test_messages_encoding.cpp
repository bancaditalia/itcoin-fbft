...
struct MessagesEncodingFixture : ReplicaStateFixture {
  MessagesEncodingFixture() : ReplicaStateFixture(4, 0, 60) {}
};
...
BOOST_FIXTURE_TEST_CASE(test_messages_encoding_00, MessagesEncodingFixture) {
  ...
  //
  {
    ...
    PrePrepare ppp_0 = PrePrepare(sender_id_ppp, v_ppp, n, req_digest, block);

    NewView msg = NewView(sender_id, v, vector<ViewChange>{vc_0}, vector<PrePrepare>{ppp_0});
