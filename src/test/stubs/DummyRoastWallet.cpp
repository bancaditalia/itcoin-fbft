...
DummyRoastWallet::DummyRoastWallet(const itcoin::FbftConfig& conf) : Wallet(conf), RoastWallet(conf) {
  ...
  PlCall("assertz",
         PlTermv(PlCompound(
             ...
             "roast_crypto_pre_sig_aggregate_dummy(Replica_id, Pre_signature_shares, Pre_signature))")));
}
...
CBlock DummyRoastWallet::FinalizeBlock(const CBlock& block, const std::string pre_sig,
                                       const std::vector<std::string> sig_shares) const {
  string msg = str(
      boost::format("FinalizeBlock Replica_id=%1%, state=%2%, presig=%3%, shares=%4%, block=%5%") %
      m_conf.id() % state_i % pre_sig % boost::algorithm::join(sig_shares, ",") % block.GetHash().GetHex());
...
std::string DummyRoastWallet::GetPreSignatureShare() {
  string presig = str(boost::format("Pre_share_%1%_%2%") % m_conf.id() % state_i);

  string msg = str(boost::format("GetPreSignatureShare Replica_id=%1%, State_i=%2%, Presig=%3%") %
                   m_conf.id() % state_i % presig);
  BOOST_LOG_TRIVIAL(trace) << msg;
...
std::string DummyRoastWallet::GetSignatureShare(std::vector<uint32_t> signers, std::string pre_signature,
                                                const CBlock& block) {
  ...
  string sig_share = str(boost::format("Sig_share_%1%_[%2%]") % m_conf.id() % signers_str);

  string msg =
      str(boost::format("GetSignatureShare Replica_id=%1%, State_i=%2%, Signers=%3%, pre_signature=%4%, "
                        "block=%5%, sigshare=%6%") %
          m_conf.id() % state_i % signers_str % pre_signature % block.GetHash().GetHex() % sig_share);
  BOOST_LOG_TRIVIAL(trace) << msg;
  return sig_share;
}
