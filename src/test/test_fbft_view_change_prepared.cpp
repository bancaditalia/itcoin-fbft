...
struct ViewChangePreparedFixture : ReplicaSetFixture {
  ViewChangePreparedFixture() : ReplicaSetFixture(4, 0, 60) {}
};

BOOST_AUTO_TEST_SUITE(test_fbft_view_change_prepared, *utf::enabled())

BOOST_FIXTURE_TEST_CASE(test_fbft_view_change_prepared_00, ViewChangePreparedFixture) {
  try {
    ...
    BOOST_LOG_TRIVIAL(debug) << "---------- Create Request with digest " << request.digest()
                             << "at R0, R1, R2, R3";
...
BOOST_AUTO_TEST_SUITE_END() // test_fbft_view_change_prepared
