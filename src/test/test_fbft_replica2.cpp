...
struct Replica2Fixture : ReplicaSetFixture {
  Replica2Fixture() : ReplicaSetFixture(4, 0, 60) {}
};

BOOST_AUTO_TEST_SUITE(test_fbft_replica2, *utf::enabled())

BOOST_FIXTURE_TEST_CASE(test_fbft_replica2_00, Replica2Fixture) {
  try {
    ...
    // In order for the algorithm to converge, three replica should work without interruption for at least
    // 16*TBT The residual time should be bigger than the max timeout, we set RECOVERY_WINDOW = 16*TBT +1*TBT
    // = 17*TBT In total, we have (15 + 17)*TBT =  total test time.
    ...
    BOOST_LOG_TRIVIAL(debug) << str(
        boost::format(
            "KILL_WINDOW = %1%, RECOVERY_WINDOW = %2%, MAX_SYNTHETIC_TIME = %3%, TARGET_HEIGHT = %4%.") %
        KILL_WINDOW % RECOVERY_WINDOW % MAX_SYNTHETIC_TIME % TARGET_HEIGHT);
    ...
    // BOOST_FAIL("Test DID NOT FAIL. This is a failure placeholder used just to print all dynamic
    // predicates");
...
BOOST_FIXTURE_TEST_CASE(test_fbft_replica2_01, Replica2Fixture) {
  try {
    boost::log::core::get()->set_filter(boost::log::trivial::severity >= boost::log::trivial::debug);
...
BOOST_AUTO_TEST_SUITE_END() // test_fbft_replica2
