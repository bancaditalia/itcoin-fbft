...
ReplicaSetFixture::ReplicaSetFixture(uint32_t cluster_size, uint32_t genesis_block_timestamp,
  ...
  for (int i = 0; i < CLUSTER_SIZE; i++) {
    ...
    std::shared_ptr<Replica2> replica =
        std::make_shared<Replica2>(*m_configs.at(i), *m_blockchain, *m_wallets.at(i), *m_transports.at(i),
                                   start_height, start_hash, start_time);
...
void ReplicaSetFixture::kill(uint32_t replica_id) {
  if (!m_transports.at(replica_id)->active) {
    BOOST_LOG_TRIVIAL(debug) << str(boost::format("R%1% is already sleeping.") % replica_id);
    return;
  }

  BOOST_LOG_TRIVIAL(info) << str(boost::format("R%1% going to sleep now.") % replica_id);
...
void ReplicaSetFixture::wake(uint32_t replica_id) {
  if (m_transports.at(replica_id)->active) {
    BOOST_LOG_TRIVIAL(debug) << str(boost::format("R%1% is already awake.") % replica_id);
    return;
  }

  BOOST_LOG_TRIVIAL(debug) << str(boost::format("R%1% wakes up.") % replica_id);
  ...
  for (size_t i = std::max((int)m_blockchain->chain.size() - 3, 0); i < m_blockchain->chain.size(); i++) {
    std::unique_ptr<Block> p_msg =
        std::make_unique<Block>(i, m_blockchain->chain[i].nTime, m_blockchain->chain[i].GetHash().GetHex());
