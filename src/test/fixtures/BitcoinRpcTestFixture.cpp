...
BitcoinRpcTestFixture::BitcoinRpcTestFixture() {
  for (int i = 0; i < CLUSTER_SIZE; i++) {
    ...
    std::unique_ptr<itcoin::transport::BtcClient> bitcoin =
        std::make_unique<itcoin::transport::BtcClient>(config->itcoin_uri());
    std::unique_ptr<itcoin::wallet::BitcoinRpcWallet> wallet =
        std::make_unique<itcoin::wallet::BitcoinRpcWallet>(*config, *bitcoin);
    std::unique_ptr<itcoin::blockchain::BitcoinBlockchain> blockchain =
        std::make_unique<itcoin::blockchain::BitcoinBlockchain>(*config, *bitcoin);
