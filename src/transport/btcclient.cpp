...
BtcClient::BtcClient(const std::string itcoinJsonRpcUri)
    : httpClient(itcoinJsonRpcUri), bitcoind(httpClient, jsonrpc::JSONRPC_CLIENT_V1) {
} // BtcClient::BtcClient()

std::string BtcClient::sendtoaddress(const std::string& address, int amount) {
  std::scoped_lock lock(this->mtx);
  return this->bitcoind.sendtoaddress(address, amount, "comment", "comment_to", false, true, "null", "unset",
                                      false, 25);
}
...
bool BtcClient::verifymessage(const std::string& address, const std::string& signature,
