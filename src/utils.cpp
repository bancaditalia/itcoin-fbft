...
std::string concatenateAndComputeHash(std::string serializedType, std::string serializedMessage) {
  ...
  sha256.add(serializedType.data(), serializedType.size());
  sha256.add(serializedMessage.data(), serializedMessage.size());

  unsigned char computedHash[SHA256::HashBytes];

  sha256.getHash(computedHash);

  return std::string(reinterpret_cast<char*>(computedHash), SHA256::HashBytes);
} // concatenateAndComputeHash()

std::string computeHash(std::string data) {
  return concatenateAndComputeHash(data, "");
} // computeHash()
...
std::vector<uint8_t> computeHash(std::vector<uint8_t> inputBuffer)
{
  ...
  // SHA256::HashBytes is the number of bytes a sha256 hash is made of, e.g. 32
  std::vector<uint8_t> binHash(SHA256::HashBytes);

  sha256.add(inputBuffer.data(), inputBuffer.size());
  sha256.getHash(binHash.data());
