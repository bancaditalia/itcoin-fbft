...
NewView::NewView(uint32_t sender_id,
  uint32_t view,
...
NewView::NewView(PlTerm Sender_id, PlTerm V, PlTerm Nu, PlTerm Chi)
:Message(NODE_TYPE::REPLICA, Sender_id)
{
  m_view = (long) V;
  ...
  for (new_view_chi_elem_t& chi_elem: chi)
  {
    ...
    PrePrepare ppp = PrePrepare(
      PlTerm((long) m_sender_id), PlTerm((long) m_view), PlTerm((long) chi_n),
      PlString((const char*) chi_digest.c_str()),
      PlString((const char*) chi_block.c_str())
    );
...
std::vector<std::unique_ptr<messages::NewView>> NewView::BuildToBeSent(uint32_t replica_id)
{
  ...
  {
    std::unique_ptr<messages::NewView> msg = std::make_unique<messages::NewView>(
      Replica_id, V, Nu, Chi
    );
...
{
  new_view_nu_t nu{};
  PlTail Nu_tail{Nu}; PlTerm Nu_elem;
  while (Nu_tail.next(Nu_elem))
  {
    if (string("[|]").compare(Nu_elem.name())!=0 || Nu_elem.arity() != 2) throw std::runtime_error("NEW_VIEW Nu element term must have arity = 2");
    uint32_t nu_n = (long) Nu_elem[1];
    PlTerm Nu_elem_2 = Nu_elem[2];
    if (string("[|]").compare(Nu_elem_2.name())!=0 || Nu_elem_2.arity() != 2) throw std::runtime_error("NEW_VIEW Nu_elem_2 term must have arity = 2");
...
{
  new_view_chi_t chi{};
  PlTail Chi_tail{Chi}; PlTerm Chi_elem;
  while (Chi_tail.next(Chi_elem))
  {
    if (string("[|]").compare(Chi_elem.name())!=0 || Chi_elem.arity() != 2) throw std::runtime_error("SEND_NEW_VIEW Chi_elem term must have arity = 2");
    uint32_t chi_n = (long) Chi_elem[1];
    PlTerm Chi_elem_2 = Chi_elem[2];

    if (string("[|]").compare(Chi_elem_2.name())!=0 || Chi_elem_2.arity() != 2) throw std::runtime_error("SEND_NEW_VIEW Chi_elem_2 term must have arity = 2");
    std::string chi_digest{ (const char*)  Chi_elem_2[1] };
    PlTerm Chi_elem_3 = Chi_elem_2[2];

    if (string("[|]").compare(Chi_elem_3.name())!=0 || Chi_elem_3.arity() != 2) throw std::runtime_error("SEND_NEW_VIEW Chi_elem_3 term must have arity = 2");
...
PlTerm NewView::nu_as_plterm(new_view_nu_t nu)
{
  ...
  for (new_view_nu_elem_t elem: nu)
  {
    ...
    Nu_tail.append( PlCompound("[|]", PlTermv(
      PlTerm((long) n),
      PlCompound("[|]", PlTermv(
...
PlTerm NewView::chi_as_plterm(new_view_chi_t chi)
{
  ...
  for (new_view_chi_elem_t elem: chi)
  {
    ...
    Chi_tail.append( PlCompound("[|]", PlTermv(
      PlTerm((long) n),
      PlCompound("[|]", PlTermv(
        PlString((const char*) digest.c_str()),
        PlCompound("[|]", PlTermv(
          PlString((const char*) prepared_block.c_str()),
          PlCompound("[]")
        ))
      ))
    )));
...
void NewView::Sign(const Wallet& wallet)
{
...
bool NewView::VerifySignatures(const Wallet& wallet)
{
  bool result = true;
...
bool NewView::equals(const Message& other) const
{
  ...
  if (m_vc_messages != typed_other.m_vc_messages) return false;
  if (m_ppp_messages != typed_other.m_ppp_messages) return false;
...
const std::string NewView::digest() const
{
  ...
  PlTermv args(
    PlTerm((long) m_view),
    NewView::nu_as_plterm(this->nu()),
    NewView::chi_as_plterm(this->chi()),
    Digest
  );
  ...
  else
  {
    string error_msg = str(
      boost::format("Unable to calculate NEW_VIEW digest")
    );
...
NewView::NewView(const Json::Value& root):
Message(root)
{
