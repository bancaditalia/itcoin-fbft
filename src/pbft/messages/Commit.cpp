...
:Message(NODE_TYPE::REPLICA, sender_id)
{
  ...
  m_block_signature = block_signature;
}
...
:Message(NODE_TYPE::REPLICA, Sender_id)
{
  ...
  this->set_block_signature(block_signature_hex);
}

void Commit::set_block_signature(std::string block_signature)
{
  m_block_signature = block_signature;
}
...
const std::string Commit::digest() const
{
  ...
  int pl_ok = PlCall("digest_commit", PlTermv(
    ...
    PlString({(const char*) block_signature().c_str()}),
