...
messages::Request Request::FindByDigest(uint32_t replica_id, std::string req_digest)
{
  ...
  else
  {
    string error_msg = str(
      boost::format("Unable to find REQUEST with digest %1%")
        % req_digest
    );
...
bool Request::TryFindByDigest(uint32_t replica_id, const std::string req_digest, messages::Request& out_request)
{
  ...
  int result =
    PlCall("msg_log_request", PlTermv(Replica_id, Req_digest, Req_timestamp)) &&
...
bool Request::equals(const Message& other) const
{
  ...
  if (m_timestamp != typed_other.m_timestamp) return false;
...
uint32_t Request::height() const
{
  return (m_timestamp-m_genesis_block_timestamp) / m_target_block_time;
}

const std::string Request::digest() const
{
  return str(
    boost::format( "(H=%1%, T=%2%)" )
      % height()
      % m_timestamp
  );
}
...
{
  return str(
    boost::format( "Request digest=%1%, timestamp=%3%" )
      % digest()
      % m_timestamp
  );
}
