...
const string DEFAULT_PBFT_DB_FILENAME = "miner.pbft.db";

PbftConfig::PbftConfig(std::string datadir, std::string configFileName)
{
  ...
  m_pbft_db_reset = false;
  m_pbft_db_filename = datadir + "/" + DEFAULT_PBFT_DB_FILENAME;

  // Clear args
  gArgs.ClearArgs();

  // Read bitcoind configuration
  gArgs.AddArg("-datadir=<dir>", "Specify data directory. The miner will read its configuration from " + configFileName + ", and the bitcoind specific data from " + miner_config_file, ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
  ...
  gArgs.AddArg("-rpcport=<port>", strprintf("Connect to JSON-RPC on <port>"), ArgsManager::ALLOW_ANY | ArgsManager::NETWORK_ONLY, OptionsCategory::OPTIONS);
  gArgs.AddArg("-rpcuser=<user>", "Username for JSON-RPC connections", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
  gArgs.AddArg("-signetchallenge=<signetchallenge>", "The signet challenge.", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
  gArgs.AddArg("-zmqpubitcoinblock=<address>", "Enable publish hash block, height and time in <address> (ITCOIN_SPECIFIC)", ArgsManager::ALLOW_ANY, OptionsCategory::ZMQ);
...

  {
    ...
    if ((consumed_chars == 0) || (consumed_chars < remaining_chars) || (itcoinblock_port > std::numeric_limits<uint16_t>::max())) {
  ...
  test_rpc_port_ = config["test_rpc_port"].asString();

  // Read signature algorithm from file
  string sig_algo = config["signature_algorithm"].asString();
  BOOST_LOG_TRIVIAL(info) << "Reading signature algorithm "<< sig_algo;
  if(sig_algo==SIGNATURE_ALGO_TYPE_AS_STRING[SIGNATURE_ALGO_TYPE::ROAST])
  {
    m_signature_algorithm = SIGNATURE_ALGO_TYPE::ROAST;
  ...
  else if(sig_algo==SIGNATURE_ALGO_TYPE_AS_STRING[SIGNATURE_ALGO_TYPE::THREEFBFT])
  {
    m_signature_algorithm = SIGNATURE_ALGO_TYPE::THREEFBFT;
    // Remove prefix 5120 from signet challenge to retrieve the group public key
    m_group_public_key = itcoin_signet_challenge_.substr(4);
  }
  else if(sig_algo==SIGNATURE_ALGO_TYPE_AS_STRING[SIGNATURE_ALGO_TYPE::NAIVE])
  {
    m_signature_algorithm = SIGNATURE_ALGO_TYPE::NAIVE;
  }
  else
  {
    std::string msg = "Could not identify signature algorithm " + sig_algo + " from configuration. Valid values are NAIVE or ROAST.";
    throw std::runtime_error(msg);
  }
  ...
  Json::Value replica_config_a = config["pbft_replica_set"];
...
const unsigned int PbftConfig::cluster_size() const {
  return m_cluster_size;
}

std::string PbftConfig::getSignetChallenge() const {
  return this->itcoin_signet_challenge_;
} // PbftConfig::getSignetChallenge()
