...
void Replica2::ApplyActiveActions() {
  ...
  uint32_t num_applied_actions = 0;
  uint32_t MAX_NUM_APPLIED_ACTIONS = 11;
    ...
    if (!m_out_msg_buffer.empty()) {
      ...
      for (auto& p_msg : ready_to_be_sent) {
        ...
        if (p_msg->type() == MSG_TYPE::ROAST_PRE_SIGNATURE) {
          unique_ptr<messages::RoastPreSignature> typed_msg =
              make_unique<messages::RoastPreSignature>(dynamic_cast<messages::RoastPreSignature&>(*p_msg));
          ...
          unique_ptr<messages::RoastSignatureShare> typed_msg = make_unique<messages::RoastSignatureShare>(
              dynamic_cast<messages::RoastSignatureShare&>(*p_msg));
          this->m_in_msg_buffer.emplace_back(std::move(typed_msg));
        }
