...
Commit::Commit(uint32_t sender_id, uint32_t view, uint32_t seq_number, std::string block_signature)
...
void Commit::set_pre_signature(std::string pre_signature_hex) { m_pre_signature = pre_signature_hex; }
...
const std::string Commit::digest() const {
  PlTerm Digest;
  int pl_ok = PlCall("digest_commit", PlTermv(PlTerm{(long)m_view}, PlTerm{(long)m_seq_number},
...
std::string Commit::identify() const {
  return str(boost::format("<%1%, V=%2%, N=%3%, S=%4%>") % name() % m_view % m_seq_number % m_sender_id);
}

// Serialization and deserialization

Commit::Commit(const Json::Value& root) : Message(root) {
