...
#ifndef ITCOIN_FBFT_MESSAGES_MESSAGES_H
#define ITCOIN_FBFT_MESSAGES_MESSAGES_H
...
public:
  ...
  static bool TryFindByDigest(uint32_t replica_id, const std::string req_digest,
...
public:
  PrePrepare(uint32_t sender_id, uint32_t view, uint32_t seq_number, std::string req_digest,
  ...
  // Finders
  static messages::PrePrepare FindByV_N_Req(uint32_t replica_id, uint32_t v, uint32_t n,
...
public:
  Prepare(uint32_t sender_id, uint32_t view, uint32_t seq_number, std::string req_digest);
...
public:
  ...
  // Getters
  std::unique_ptr<Message> clone();
  const std::string pre_signature() const { return m_pre_signature; }
  const std::string digest() const;
  std::string identify() const;
  uint32_t seq_number() const { return m_seq_number; }
  std::optional<uint32_t> seq_number_as_opt() const { return m_seq_number; }
...
private:
  ...
  bool equals(const Message& other) const;
  void set_pre_signature(std::string pre_signature_hex);
};
...
public:
  ViewChange(uint32_t sender_id, uint32_t view, uint32_t hi, std::string c, view_change_prepared_t pi,
             view_change_pre_prepared_t qi);
  ViewChange(PlTerm Sender_id, PlTerm V, PlTerm Hi, PlTerm C, PlTerm Pi, PlTerm Qi);
...
public:
  NewView(uint32_t sender_id, uint32_t view, std::vector<ViewChange> vc_messages,
          std::vector<PrePrepare> ppp_messages);
  NewView(PlTerm Sender_id, PlTerm V, PlTerm Nu, PlTerm Chi);
  ...
  // Operations
  void Sign(const wallet::RoastWallet& wallet);
  bool VerifySignatures(const wallet::RoastWallet& wallet);
...
public:
  RoastPreSignature(uint32_t sender_id, std::vector<uint32_t> signers, std::string pre_signature);
  RoastPreSignature(PlTerm Sender_id, PlTerm Signers, PlTerm Pre_signature);
...
public:
  RoastSignatureShare(uint32_t sender_id, std::string signature_share, std::string next_pre_signature_share);
  RoastSignatureShare(PlTerm Sender_id, PlTerm Signature_share, PlTerm Next_pre_signature_share);
...
#endif // ITCOIN_FBFT_MESSAGES_MESSAGES_H
