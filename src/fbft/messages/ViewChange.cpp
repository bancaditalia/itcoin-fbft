...
ViewChange::ViewChange(uint32_t sender_id,
...
ViewChange::ViewChange(PlTerm Sender_id,
...
:Message(NODE_TYPE::REPLICA, Sender_id)
{
  ...
  PlTail P_tail{Pi}; PlTerm P_elem;
  while (P_tail.next(P_elem))
  {
    if (string("[|]").compare(P_elem.name())!=0 ||  P_elem.arity() != 2) throw std::runtime_error("VIEW_CHANGE message P_elem term must have arity = 2");
    uint32_t p_n = (long) P_elem[1];
    PlTerm P_elem_2 = P_elem[2];

    if (string("[|]").compare(P_elem_2.name())!=0 ||  P_elem_2.arity() != 2) throw std::runtime_error("VIEW_CHANGE message P_elem_2 term must have arity = 2");
    std::string p_req_digest{ (char*)  P_elem_2[1] };
    PlTerm P_elem_3 = P_elem_2[2];

    if (string("[|]").compare(P_elem_3.name())!=0 ||  P_elem_3.arity() != 2) throw std::runtime_error("VIEW_CHANGE message P_elem_3 term must have arity = 2");
  ...
  PlTail Q_tail{Qi}; PlTerm Q_elem;
  while (Q_tail.next(Q_elem))
  {
    if (string("[|]").compare(Q_elem.name())!=0 || Q_elem.arity() != 2) throw std::runtime_error("VIEW_CHANGE message Q_elem term must have arity = 2");
    uint32_t q_n = (long) Q_elem[1];
    PlTerm Q_elem_2 = Q_elem[2];

    if (string("[|]").compare(Q_elem_2.name())!=0 || Q_elem_2.arity() != 2) throw std::runtime_error("VIEW_CHANGE message Q_elem_2 term must have arity = 2");
    std::string q_req_digest{ (char*)  Q_elem_2[1] };
    PlTerm Q_elem_3 = Q_elem_2[2];

    if (string("[|]").compare(Q_elem_3.name())!=0 || Q_elem_3.arity() != 2) throw std::runtime_error("VIEW_CHANGE message Q_elem_3 term must have arity = 2");
    std::string q_prep_block{ (char*)  Q_elem_3[1] };
    PlTerm Q_elem_4 = Q_elem_3[2];

    if (string("[|]").compare(Q_elem_4.name())!=0 || Q_elem_4.arity() != 2) throw std::runtime_error("VIEW_CHANGE message Q_elem_4 term must have arity = 2");
...
std::vector<std::unique_ptr<messages::ViewChange>> ViewChange::BuildToBeSent(uint32_t replica_id)
{
  ...
  {
    std::unique_ptr<messages::ViewChange> msg = std::make_unique<messages::ViewChange>(
      Replica_id, V, Hi, C, Pi, Qi
    );
...
messages::ViewChange ViewChange::FindByDigest(uint32_t replica_id, uint32_t sender_id, std::string digest)
{
  ...
  int result = PlCall("msg_log_view_change", PlTermv(Replica_id, Vc_digest, V, Hi, C, Pi, Qi, Sender_id, Sender_sig));
  if (result)
  {
  ...
  else
  {
    string error_msg = str(
      boost::format("Unable to find VIEW_CHANGE with digest %1%")
        % digest
    );
...
bool ViewChange::equals(const Message& other) const
{
  ...
  if (m_hi != typed_other.m_hi) return false;
  if (m_c != typed_other.m_c) return false;
  if (m_pi != typed_other.m_pi) return false;
  if (m_qi != typed_other.m_qi) return false;
...
PlTerm ViewChange::pi_as_plterm() const
{
  ...
  for (messages::view_change_prepared_elem_t elem: m_pi)
  {
    ...
    Pi_tail.append( PlCompound("[|]", PlTermv(
      PlTerm((long) n),
      PlCompound("[|]", PlTermv(
        PlString((const char*) digest.c_str()),
        PlCompound("[|]", PlTermv(
          PlTerm((long) v),
          PlCompound("[]")
        ))
...
PlTerm ViewChange::qi_as_plterm() const
{
  ...
  for (messages::view_change_pre_prepared_elem_t elem: m_qi)
  {
    ...
    Qi_tail.append( PlCompound("[|]", PlTermv(
      PlTerm((long) n),
      PlCompound("[|]", PlTermv(
        PlString((const char*) digest.c_str()),
        PlCompound("[|]", PlTermv(
          PlString((const char*) prep_block.c_str()),
          PlCompound("[|]", PlTermv(
            PlTerm((long) v),
            PlCompound("[]")
          ))
...
const std::string ViewChange::digest() const
{
  ...
  {
    string error_msg = str(
      boost::format("Unable to calculate VIEW_CHANGE digest")
    );
    throw(std::runtime_error(error_msg));
  }
}
...
ViewChange::ViewChange(const Json::Value& root):
Message(root)
{
  ...
  std::transform(
    qi.begin(), qi.end(),
    std::back_inserter(m_qi), [](const auto& e) {
