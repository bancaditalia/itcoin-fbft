...
std::vector<std::unique_ptr<messages::RoastPreSignature>> RoastPreSignature::BuildToBeSent(uint32_t replica_id)
{
  ...
  {
    std::unique_ptr<messages::RoastPreSignature> msg = std::make_unique<messages::RoastPreSignature>(
      Replica_id, Signers, Pre_signature
    );
...
const std::string RoastPreSignature::digest() const
{
  ...
  PlTermv args(
    signers_as_plterm(),
    PlString((const char *) m_pre_signature.c_str()),
...
bool RoastPreSignature::equals(const Message& other) const
{
  ...
  if (m_signers != typed_other.m_signers) return false;
  if (m_pre_signature != typed_other.m_pre_signature) return false;
...
std::string RoastPreSignature::identify() const
{
  ...
  return str(
    boost::format( "<%1%, Signers=[%2%], Pre_sig=%3%, S=%4%>" )
      % name()
      % signers_str
      % m_pre_signature.substr(0,5)
      % m_sender_id
  );
}

std::string RoastPreSignature::pre_signature() const
{
  return m_pre_signature;
}

std::vector<uint32_t> RoastPreSignature::signers() const
{
  return m_signers;
}
...
RoastPreSignature::RoastPreSignature(const Json::Value& root):
Message(root)
{
