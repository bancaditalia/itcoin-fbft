...
string Message::name() const { return MSG_TYPE_AS_STRING[static_cast<uint32_t>(this->type())]; }

std::optional<uint32_t> Message::seq_number_as_opt() const { return std::nullopt; }
...
std::string Message::signature() const { return this->m_signature; }

void Message::set_signature(std::string signature) { this->m_signature = signature; }

// Operations

void Message::Sign(const Wallet& wallet) { wallet.AppendSignature(*this); }

bool Message::VerifySignatures(const Wallet& wallet) { return wallet.VerifySignature(*this); }

//

bool Message::operator==(const Message& other) const { return this->equals(other); }
...
optional<unique_ptr<Message>> Message::BuildFromBinBuffer(const std::string& bin_buffer) {
  ...
  if (!parsing_ok) {
    string error_msg = str(boost::format("Message::BuildFromBinBuffer unable parse json: %1%.") % bin_buffer);
    ...
    if (msg_type == MSG_TYPE::COMMIT) {
      ...
      string error_msg =
          str(boost::format("Message::BuildFromBinBuffer unable to identify message type in json: %1%.") %
              bin_buffer);
      BOOST_LOG_TRIVIAL(error) << error_msg;
    }
  }
