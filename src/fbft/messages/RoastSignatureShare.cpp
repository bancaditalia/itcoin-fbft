...
RoastSignatureShare::RoastSignatureShare(uint32_t sender_id, string signature_share,
                                         string next_pre_signature_share)
    : Message(NODE_TYPE::REPLICA, sender_id) {
  m_signature_share = signature_share;
  m_next_pre_signature_share = next_pre_signature_share;
}

RoastSignatureShare::RoastSignatureShare(PlTerm Sender_id, PlTerm Signature_share,
                                         PlTerm Next_pre_signature_share)
    : Message(NODE_TYPE::REPLICA, Sender_id) {
...
std::vector<std::unique_ptr<messages::RoastSignatureShare>>
RoastSignatureShare::BuildToBeSent(uint32_t replica_id) {
...
const std::string RoastSignatureShare::digest() const {
  PlTerm Digest;
  PlTermv args(PlString((const char*)m_signature_share.c_str()),
               PlString((const char*)m_next_pre_signature_share.c_str()), PlTerm{(long)m_sender_id}, Digest);
...
std::string RoastSignatureShare::identify() const {
  return str(boost::format("<%1%, Sig_share=%2%, Next_pre_sig_share=%3%, S=%4%>") % name() %
             m_signature_share.substr(0, 5) % m_next_pre_signature_share.substr(0, 5) % m_sender_id);
}

std::string RoastSignatureShare::signature_share() const { return m_signature_share; }

std::string RoastSignatureShare::next_pre_signature_share() const { return m_next_pre_signature_share; }

// Serialization and deserialization

RoastSignatureShare::RoastSignatureShare(const Json::Value& root) : Message(root) {
