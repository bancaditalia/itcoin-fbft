...
bool Block::equals(const Message& other) const
{
  ...
  if (m_block_height != typed_other.m_block_height) return false;
  if (m_block_time != typed_other.m_block_time) return false;
  if (m_block_hash != typed_other.m_block_hash) return false;
...
{
  return str(
    boost::format( "<BLOCK, height=%1%, time=%2%, hash=%3%>" )
      % m_block_height
      % m_block_time
      % m_block_hash
  );
