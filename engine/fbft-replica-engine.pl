...
% This file implements a FBFT I/O Automata from [1] using Constraint Logic Programming over Finite Domains - CLP(FD)
...
% - The VIEW_CHANGE_ACK message is not present, since view change messages are authenticated
...
print_all_dynamics :-
  ...
  writef(":- constants.\n \nRequest_buffer_len=%w \nCluster_size=%w \nGenesis_block_timestamp=%w \nTarget_block_time=%w \n\n", [L, Cluster_size, Genesis_block_timestamp, Target_block_time]),
  ...
  ( % if
    ...
    (
      ...
      foreach(
        ...
        % The coordinator (primary) receives pre-signature shares via associated commit data
        % There is no signature_share yet at this point (= false)
