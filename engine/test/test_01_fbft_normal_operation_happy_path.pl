...
:- begin_tests(test_01_fbft_normal_operation_happy_path).
...
% All replicas receive commit and get ready to enter ROAST operations
test(test_happy_path_08, setup(build_config(Config))) :-
  ...
  active_ROAST_INIT(0, Active_roast_init_0),
  assertion( Active_roast_init_0 == [[Config.req_digest_1, V, N]]),
  ...
  active_ROAST_INIT(1, Active_roast_init_1),
  assertion( Active_roast_init_1 == [[Config.req_digest_1, V, N]]),
  ...
  active_ROAST_INIT(2, Active_roast_init_2),
  assertion( Active_roast_init_2 == [[Config.req_digest_1, V, N]]),
  ...
  active_ROAST_INIT(3, Active_roast_init_3),
  assertion( Active_roast_init_3 == [[Config.req_digest_1, V, N]]).
  % active_EXECUTE(3, Active_execute_3),
  % assertion( Active_execute_3 == [[Config.req_digest_1, V, N]]).

setup_roast() :-
assertz_once(roast_crypto_pre_sig_aggregate(0, ["DATA","DATA"], "PRE_SIG1")).

test(test_happy_path_roast, [setup((setup_roast(),build_config(Config)))]) :-
  V = 0, N = 1,
  foreach(
    between(0,3, Replica_id),
    apply_ROAST_INIT(Replica_id, V, N, Config.req_digest_1)).

/* TODO
  1) primary sends PRE_SIG to replicas
  2) replica send signature_share and new pre_signature_share to primary
  3) if enough send, pre_EXECUTE should be true
  4) primary executes */

% all replicas execute TODO: only primary executes
...
:- end_tests(test_01_fbft_normal_operation_happy_path).
