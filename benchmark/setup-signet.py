...
from benchmark_framework.utils import import_module_by_abspath, byzantine_quorum, byzantine_replies_quorum, \
    get_node_dirname, SignatureAlgo, NodeType
...
REPLICA_TEST_RPC_PORT_START = 12000
PBFT_REPLICA_SET_PORT_START = 13000
PBFT_CLIENT_SET_PORT_START = 14000
RPC_BIND = "127.0.0.1"
...
class SignetDirectoryBuilder:
    ...
    def __init__(
        ...
        self._frost_config = sig_algo == SignatureAlgo.ROAST or sig_algo == SignatureAlgo.THREEFBFT
        self._sig_algo = sig_algo
    ...
    def _declare_itcoin_test_case_class(self) -> Type[BitcoinTestFramework]:
        ...
        class MyItcoinTestCase(test_framework_itcoin_module.BaseItcoinTest):
            def set_test_params(self):
                self.num_nodes = nb_validators + nb_clients
                self.signet_num_signers = nb_validators
                self.signet_num_signatures = byzantine_quorum(nb_validators)
        ...
        class MyThreeFBFTTestCase(test_framework_itcoin_frost_module.BaseFrostTest):
            def set_test_params(self):
                self.num_nodes = nb_validators + nb_clients
                self.signet_num_signers = nb_validators
                self.signet_num_signatures = byzantine_quorum(nb_validators)
        ...
        return MyFrostItcoinTestCase if self._sig_algo == SignatureAlgo.ROAST \
            else MyItcoinTestCase if self._sig_algo == SignatureAlgo.NAIVE \
            else MyThreeFBFTTestCase if self._sig_algo == SignatureAlgo.THREEFBFT \
            else None
    ...
    def _create_miner_config(self, node_id, signet_key_pairs, frost_info, use_remote_config : bool = False):
        ...
        config = {
            "id": node_id,
            "test_rpc_port": REPLICA_TEST_RPC_PORT_START if use_remote_config else REPLICA_TEST_RPC_PORT_START + node_id,
            ...
            "signature_algorithm": "NAIVE" if self._sig_algo==SignatureAlgo.NAIVE else "ROAST" if self._sig_algo==SignatureAlgo.ROAST else "THREEFBFT" if self._sig_algo==SignatureAlgo.THREEFBFT else None,
            "pbft_replica_set": [
                {
                    "host": self._remote_config[replica_id] if use_remote_config else LOCALHOST,
                    "port": PBFT_REPLICA_SET_PORT_START if use_remote_config else PBFT_REPLICA_SET_PORT_START + replica_id,
                    "p2pkh": address,
                    "pubkey": getattr(frost_info[replica_id],'pubkey') if self._frost_config else None
                }
    ...
    def build(self) -> None:
        ...
        with save_argv(), bitcoin_testcase_network(
            ...
            frost_info = test_case_object.frost_info if self._frost_config else None
...
@click.option(
    ...
    type=str,
    required=True,
    help="Which sig algo to use, NAIVE, ROAST, THREEFBFT.",
)
@click.option(
...
def main(output_directory: str, itcoin_core_dir: str, nb_validators: int, nb_clients: int, sig_algo: str,
genesis_block_timestamp: int, target_block_time: int, remote_config: str) -> None:
    """Set up a Signet network configuration directory."""
    sig_algo_as_enum: SignatureAlgo = SignatureAlgo.from_string(sig_algo)
