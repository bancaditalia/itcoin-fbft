...
from benchmark_framework.config import BenchParameters, DEFAULT_BENCH_PARAMETERS_FILENAME, DEFAULT_SUMMARY_FILENAME
from benchmark_framework.logs import RunResult, ParseError
from benchmark_framework.paths import PathMaker
from benchmark_framework.utils import Print, import_module_by_abspath, get_node_dirname, NodeType, BenchError, \
...
class Bench(ABC):
    ...
    def __init__(self,
        ...
        :param skip_build: whether the local build of itcoin-pbft should be skipped or not.
    ...
    def _build_local(self) -> None:
        ...
        if not self.skip_build:
            options = '-DCMAKE_C_COMPILER=gcc-10 -DCMAKE_CXX_COMPILER=g++-10 -DITCOIN_CORE_SRC_DIR=../../itcoin-core ' \
    ...
    def _get_log_dirname(self, start_time: str):
        return f'{start_time}-{self.bench_parameters.faults}-' \
               f'{self.bench_parameters.nodes}-{self.bench_parameters.rate}-' \
               f'{self.bench_parameters.tx_size}'
