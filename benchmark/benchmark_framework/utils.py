...
class SignatureAlgo(Enum):
    NAIVE = "NAIVE"
    ROAST = "ROAST"
    THREEFBFT = "THREEFBFT"

    @staticmethod
    def from_string(signature_algorithm):
        assert isinstance(signature_algorithm, str)
        sig_algo: SignatureAlgo = None
        if signature_algorithm in ('NAIVE', 'naive', 'Naive'):
            sig_algo = SignatureAlgo.NAIVE
        elif signature_algorithm in ('ROAST', 'roast', 'Roast'):
            sig_algo = SignatureAlgo.ROAST
        elif signature_algorithm in ("THREEFBFT"):
            sig_algo = SignatureAlgo.THREEFBFT
        else:
...
class SignatureAlgoField(CustomField[str]):
    def __init__(self, default=MISSING, validators: Sequence[Callable[[Any, T], None]] = ()) -> None:
        """Initialize a SignatureAlgo field."""
        super().__init__(type_=str, default=default, validators=validators)
