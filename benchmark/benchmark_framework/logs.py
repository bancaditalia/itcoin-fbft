...
from benchmark_framework.utils import PathLike, SignatureAlgo, NodeType
...
class TimeIntervals:
    ...
    def get_time_spent_summary(self, not_before: PosixTimestamp = 0) -> str:
        ...
        time_spent_for_pbft = time_by_type[TimeIntervalType.PBFT]
        time_spent_for_roast = time_by_type[TimeIntervalType.ROAST]
        time_spent_for_bitcoin = time_by_type[TimeIntervalType.BITCOIN]
        total_time_spent = time_spent_for_pbft + time_spent_for_roast + time_spent_for_bitcoin
        return \
            f' Time spent for PBFT:                {round(time_spent_for_pbft * 1000):,} ms ({round(time_spent_for_pbft / total_time_spent * 100, ndigits=2)} %)\n' \
...
class NodeLog:
    ...
    def parse_time_intervals(self) -> Optional[TimeIntervals]:
        ...
        for start_pbft_event, start_roast, execute, start_submitblock, end_submitblock in zip(*events_split_by_type):
            assert start_pbft_event.block_height == start_roast.block_height == execute.block_height == start_submitblock.block_height == end_submitblock.block_height
            assert start_pbft_event.log_timestamp <= start_roast.log_timestamp <= execute.log_timestamp <= start_submitblock.log_timestamp <= end_submitblock.log_timestamp
            height = start_pbft_event.block_height
            pbft_time_interval = TimeInterval(
                start_pbft_event.log_timestamp,
                start_roast.log_timestamp,
                TimeIntervalType.PBFT
            )
            ...
            result[height] = {interval.interval_type: interval for interval in [pbft_time_interval, submitblock_time_interval, roast_time_interval]}
...
class RunResult:
    ...
    def time_intervals(self) -> Optional[TimeIntervals]:
        ...
        if primary_logs is None:
            return None
        if not self.bench_parameters.signature_algorithm==SignatureAlgo.ROAST.name:
            return None
    ...
    def csv_summary(self, filename) -> None:
        ...
        data = [self.bench_parameters.nodes,
            ...
            self.bench_parameters.signature_algorithm,
            ...
            time_spent_by_protocol[TimeIntervalType.PBFT],
    ...
    def csv_summary_by_height(self, filename: str) -> None:
        ...
        for height in heights:
            ...
            data.append([
                ...
                request_latency,
                preprepare_latency,
                time_spent_by_protocol[TimeIntervalType.PBFT].diff if time_spent_by_protocol is not None else float('nan'),
    ...
    def summary(self) -> str:
        ...
        time_intervals_summary = time_intervals.get_time_spent_summary(self.node_logs.start + self.bench_parameters.warmup_duration) if time_intervals else "No time intervals summary available (view change happened, old logs not containing EndSubmitBlock event, or signature_algorithm != ROAST).\n"
        return (
            '\n'
            '-----------------------------------------\n'
            ' SUMMARY:\n'
            ...
            f' Protocol: { self.bench_parameters.signature_algorithm } \n'
...
class ExperimentResult:
    ...
    def from_directory(cls, experiment_directory: Path) -> "ExperimentResult":
        ...
        for run_directory in sorted(experiment_directory.iterdir()):
            log_results = LogParser.process(run_directory).result()
...
class LogParser:
    ...
    def __init__(self, clients: List[str], nodes: List[str], bench_parameters: BenchParameters):
    ...
    def result(self) -> RunResult:
        ...
        # The result filename is not really relevant for aggregation, using this convention just for better readability
        result.csv_summary(PathMaker.csv_result_file())
        result.csv_summary_by_height(PathMaker.csv_by_height_result_file())
        return result
    ...
    @classmethod
    def process(cls, directory: PathLike):
        directory = Path(directory)

        clients = []
        for filename in sorted(glob(join(directory, "client*/client_logs.txt"))):
            with open(filename, 'r') as f:
                clients += [f.read()]

        nodes = []
        # note, 'sorted' works because of alphanumerical ordering
        for filename in sorted(glob(join(directory, "node*/miner_logs.txt"))):
            with open(filename, 'r') as f:
                nodes += [f.read()]

        bench_parameters = BenchParameters.from_file(directory / DEFAULT_BENCH_PARAMETERS_FILENAME)
        return cls(clients, nodes, bench_parameters)
